...
describe('transaction reducer', () => {
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.HYPRA]?.['0x0'];
      expect(tx?.summary).toEqual('hello world');
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.ETHEREUM), String(ChainId.HYPRA)]);
      expect(Object.keys(store.getState()[ChainId.ETHEREUM] ?? {})).toEqual(['0x0']);
      expect(Object.keys(store.getState()[ChainId.HYPRA] ?? {})).toEqual(['0x1']);
